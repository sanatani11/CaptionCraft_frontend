    ...
    openai
      .createChatCompletion({
        ...
        messages: [
          {
            role: "user",
            content: `Write a ${caption.length} caption for ${caption.platform} post mentioning ${caption.description}. Write the caption in ${caption.mood} mood and do not include the hashtags.`,
          },
        ],
      })
      .then((res) => {
        ...
        // Determine the amount based on the length of the caption
        let amount;
        if (caption.length == "Short") {
          amount = 5;
        } else if (caption.length == "Medium") {
          amount = 8;
        } else {
          amount = 10;
        }

        // Call the reduce API with the determined amount
        reduceAmount(amount);
      });
    console.log("done");
  };

  const reduceAmount = async (amount) => {
    try {
      // Make POST request to the backend API endpoint
      let response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/auth/reduce-coin/${
      ...
      if (response.ok) {
        ...
        setCaption({
          ...caption,
          caption: "Sorryyyy....some error has occured",
        });
      }
    } catch (error) {
      console.error("Error:", error.message);
      setCaption({ ...caption, caption: "Sorryyyy....some error has occured" });
    }
  };
